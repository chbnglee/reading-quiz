/* Quiz v3 override: story-flow questions + 6 Story Grammar axes */

const V3_COUNT = 6;
const V3_SG = [
  { key: "setting", label: "Setting", kor: "배경 이해", q: 1 },
...
const hintUsed = Array(V3_COUNT).fill(false);
...
Object.assign(QD.q01, {
  ...
  slots: {
    ...
    what: { label: "At first...", correct: "loves_colors", weight: 1.5 },
  },
  cards: {
...
while (scores.length < V3_COUNT) scores.push(null);
while (answered.length < V3_COUNT) answered.push(false);
while (mcqSel.length < V3_COUNT) mcqSel.push(null);
...
function injectV2Style() {
  ...
  style.textContent = `
    ...
    .q-screen{justify-content:flex-start;align-items:center;padding-left:16px;padding-right:16px;box-sizing:border-box;background-image:linear-gradient(rgba(31,41,55,.22),rgba(31,41,55,.26)),linear-gradient(rgba(255,255,255,.48),rgba(245,243,255,.56)),url("Image/OG0021_Talking_BG_I.png")!important;background-size:cover!important;background-position:center!important;background-attachment:fixed}
    ...
    .setting-bank{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;padding:12px;background:#FFFBEB;border-radius:22px;border:2px solid #FDE68A}
    .setting-card{min-height:52px;background:white;border:2.5px solid #FCD34D;border-radius:18px;padding:10px 12px;cursor:grab;user-select:none;box-shadow:0 2px 6px rgba(0,0,0,.06);transition:all .2s;display:flex;align-items:center;justify-content:center}
    .setting-card:hover{transform:translateY(-2px);border-color:#A78BFA}.setting-word{font-family:'ABeeZee',sans-serif;font-size:16px;color:#374151;text-align:center}
    ...
    .conseq-bank{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:12px;padding:14px;margin-top:14px;background:#FFFBEB;border-radius:20px;border:2px solid #FDE68A}
    .conseq-card{grid-column:span 2;aspect-ratio:16/10;border:2.5px solid #FCD34D;border-radius:18px;overflow:hidden;cursor:grab;background:white;box-shadow:0 2px 6px rgba(0,0,0,.06);transition:all .2s}
    ...
    .bookey-btn{width:58px;height:58px;border:none;border-radius:50%;background:white;box-shadow:0 8px 24px rgba(124,58,237,.18);cursor:pointer;padding:2px;border:2px solid #EDE9FE;transition:transform .15s;overflow:hidden}
    .bookey-btn:hover{transform:translateY(-2px) scale(1.03)}
    .bookey-btn img{width:100%;height:100%;object-fit:contain;object-position:center;display:block;transform:none}
    ...
    @media(max-width:760px){.setting-slots,.setting-bank,.score-cards,.conseq-slots,.conseq-bank{grid-template-columns:1fr}.conseq-card,.conseq-card:nth-child(4),.conseq-card:nth-child(5){grid-column:auto}.ox-grid{grid-template-columns:repeat(3,1fr)}.hint-bubble{max-width:220px;font-size:13px}.bookey-btn{width:52px;height:52px}}
  `;
...
function toggleHint() {
  const widget = document.getElementById("bookey-hint");
  widget?.classList.toggle("open");
  if (widget?.classList.contains("open") && currentQ >= 0 && currentQ < V3_COUNT) {
...
function updateHint() {
  ...
  const isQuiz = activeId.startsWith("screen-q") && Number.isInteger(currentQ) && currentQ >= 0 && currentQ < V3_COUNT;
  widget.classList.toggle("show", isQuiz);
  widget.classList.remove("open");
  if (isQuiz) bubble.textContent = V3_HINTS[currentQ];
}

function injectV2Dom() {
  document.querySelector(".cover-title").insertAdjacentHTML("afterend", '<div class="cover-version">Quiz v3 · Story Grammar</div>');
  document.body.insertAdjacentHTML("beforeend", `
    <div class="bookey-hint" id="bookey-hint">
      <button class="bookey-btn" type="button" onclick="toggleHint()" aria-label="Show hint">
        <img src="../Assets/BKTK_Characters_Bookey.png" alt="Bookey">
      </button>
      <div class="hint-bubble" id="hint-bubble"></div>
    </div>`);

  document.querySelector("#screen-q0 .q-card").innerHTML = `
    <div class="q-header"><div class="q-num-badge" style="background:#BE185D">Q1</div><div class="q-instruction">Put the story scenes in order.</div><div class="q-type-tag" style="background:#BE185D">Consequence</div></div>
  ...
  document.querySelector("#screen-q1 .q-card").innerHTML = `
    <div class="q-header"><div class="q-num-badge" style="background:#7C3AED">Q2</div><div class="q-instruction">Look at the first scene. Fill in the boxes.</div><div class="q-type-tag" style="background:#7C3AED">Setting</div></div>
    <div class="scene-strip"><div class="scene-thumb">${cardImg("OG0021_SC01_I.png", "SC01")}</div></div>
    <div class="setting-slots">
      ...
      <div class="setting-slot" data-slot="what" ondragover="dragOver(event)" ondrop="dropSetting(event)"><span class="slot-label">At first...</span>Drop here</div>
    </div>
  ...
  document.querySelector("#screen-q5 .q-card").innerHTML = `
    ...
    ${nav(5, "See Results! 🎉", "showStudentResults()")}`;

  document.querySelector("#screen-parent .parent-title").textContent = "📋 학부모 리포트 — OG0021 v3";
...
function dropSetting(e) {
  ...
  v2SettingDrag = null;
  v2SettingFrom = null;
  document.getElementById("chk1").disabled = Object.values(v2SettingSlots).filter(Boolean).length < 3;
...
function settingCardHtml(key) {
  ...
  return `<div class="setting-card" draggable="true" data-key="${key}" ondragstart="startSetting(event)">
    <div class="setting-word">${card.text}</div>
  </div>`;
}
...
function renderSettingSlot(slot) {
  ...
  if (key) {
    ...
    el.innerHTML = `<span class="slot-label">${label}</span><div class="setting-word">${card.text}</div>`;
  } else {
...
function returnSettingCard(slot) {
  ...
  if (!key) return;
  const bank = document.getElementById("setting-bank");
  bank?.insertAdjacentHTML("beforeend", settingCardHtml(key));
...
function dropConseq(e) {
  ...
  v2ConseqDrag = null;
  v2ConseqFrom = null;
  document.getElementById("chk0").disabled = v2ConseqOrder.filter(Boolean).length < QD.q02.correct.length;
...
goNext = function(qIdx) {
  const next = Math.min(qIdx + 1, V3_COUNT - 1);
  jumpTo(next);
};
...
submitQ = function(qIdx) {
  ...
  if (qIdx === 0) {
    ...
    score = Math.round(earned / total * 100);
    answerLog.q02 = { slots: { ...v2SettingSlots }, score };
  ...
  const logKey = ["q01", "q02", "q03", "q04", "q05", "q06"][qIdx];
  if (answerLog[logKey]) answerLog[logKey].hintUsed = hintUsed[qIdx];
...
showParentResults = function() {
  ...
  const risks = V3_SG.filter(d => sg[d.key] < 70).map(d => `• ${d.kor}: ${feedbackLine(d.key, sg[d.key])}`);
  document.getElementById("parent-risk-body").innerHTML = risks.length ? risks.join("<br>") : "✅ 6개 Story Grammar 영역이 모두 안정적입니다.";
  showScreen("screen-parent");
};

function renderRadar(sg) {
  const vals = V3_SG.map(d => sg[d.key] || 0);
...
downloadDevSpec = function() {
  ...
  const questionRows = [[
    ...
    "report_bucket", "hint_en"
  ]];
  [
    ["OG0021_V3_Q01", 1, "STORY_SEQUENCE_DRAG", "Story Scene Sequence", "consequence", "Consequence", "Put the story scenes in order.", "OG0021_SC01_I.png, OG0021_SC02_I.png, OG0021_SC03_I.png, OG0021_SC06_I.png, OG0021_SC09_I.png", "-", QD.q02.correct.join(", "), "weighted_position", 100, "radar", V3_HINTS[0]],
    ["OG0021_V3_Q02", 2, "SETTING_SLOT_DRAG", "Setting Slot Drag", "setting", "Setting", "Look at the first scene. Fill in the boxes.", "OG0021_SC01_I.png", "-", "who=chameleon, where=forest, at_first=loves changing colors", "weighted_slot_match", 100, "radar", V3_HINTS[1]],
    ["OG0021_V3_Q03", 3, "LISTEN_SCENE_MCQ", "Listening Scene Match", "initiating_event", "Initiating Event", "Listen. Which scene starts the problem?", "OG0021_SC02_I.png, OG0021_SC03_I.png, OG0021_SC06_I.png, OG0021_SC09_I.png", QD.q03.audioSrc, QD.q03.correct, "fixed_option_score", 100, "radar", V3_HINTS[2]],
    ["OG0021_V3_Q04", 4, "SCENE_WORD_DRAG", "Scene-Anchored Unscramble", "attempt", "Attempt", "Put the story words in order.", "OG0021_SC03_I.png", "-", QD.q04.correct.join(" "), "weighted_word_position", 100, "radar", V3_HINTS[3]],
    ["OG0021_V3_Q05", 5, "EMOTION_MCQ", "Emotion Match", "reaction", "Reaction", "How does Milo feel here?", "OG0021_SC06_I.png", "-", QD.q05.correct, "fixed_option_score", 100, "radar", V3_HINTS[4]],
    ["OG0021_V3_Q06", 6, "INTERNAL_RESPONSE_MCQ", "Internal Response MCQ", "internal_response", "Internal Response", "What is Milo thinking?", "OG0021_SC06_I.png", "-", QD.q06.correct, "fixed_option_score", 100, "radar", V3_HINTS[5]],
  ...
  wsQ["!cols"] = [{ wch: 16 }, { wch: 9 }, { wch: 9 }, { wch: 22 }, { wch: 28 }, { wch: 20 }, { wch: 22 }, { wch: 42 }, { wch: 58 }, { wch: 34 }, { wch: 54 }, { wch: 24 }, { wch: 10 }, { wch: 18 }, { wch: 46 }];
  ...
  QD.q02.correct.forEach((scene, idx) => optionRows.push(["OG0021_V3_Q01", scene, QD.q02.scenes[scene].text, "position_" + (idx + 1), QD.q02.weights[scene], "", "TRUE", QD.q02.scenes[scene].res, ""]));
  Object.entries(QD.q01.cards).forEach(([key, card]) => optionRows.push(["OG0021_V3_Q02", key, card.text, card.slot === "what" ? "at_first" : card.slot, QD.q01.slots[card.slot]?.weight || "", "", key === QD.q01.slots[card.slot]?.correct ? "TRUE" : "FALSE", "-", card.w || ""]));
  [["A", "OG0021_SC02_I.png"], ["B", "OG0021_SC03_I.png"], ["C", "OG0021_SC06_I.png"], ["D", "OG0021_SC09_I.png"]].forEach(([key, res]) => {
    const opt = QD.q03.opts[key];
    optionRows.push(["OG0021_V3_Q03", key, res.replace(".png", ""), "", "", opt.s, key === QD.q03.correct ? "TRUE" : "FALSE", res, opt.w || ""]);
  });
  QD.q04.correct.forEach((word, idx) => optionRows.push(["OG0021_V3_Q04", String(idx + 1), word, "word_position_" + (idx + 1), QD.q04.weights[word], "", "TRUE", "-", ""]));
  const q05Text = { A: "Happy", B: "Sad", C: "Angry", D: "Surprised" };
  Object.entries(QD.q05.opts).forEach(([key, opt]) => optionRows.push(["OG0021_V3_Q05", key, q05Text[key], "", "", opt.s, key === QD.q05.correct ? "TRUE" : "FALSE", "OG0021_SC06_I.png", opt.w || ""]));
  Object.entries(QD.q06.opts).forEach(([key, opt]) => optionRows.push(["OG0021_V3_Q06", key, opt.text, "", "", opt.s, key === QD.q06.correct ? "TRUE" : "FALSE", "-", opt.w || ""]));
  ...
  const scoringRows = [
    ...
    ["OG0021_V3_Q01", "weighted_position", "score = round(sum(weight_i * max(0, 1 - abs(placed_pos_i - correct_pos_i) * 0.5)) / sum(weights) * 100)", "Consequence = Q01 score", "이야기 전체 사건 전개를 대표 장면 순서로 배열"],
    ["OG0021_V3_Q02", "weighted_slot_match", "full credit if card matches slot target; 35% slot credit if same category but wrong card", "Setting = Q02 score", "첫 장면을 보고 Who/Where/At first를 구성"],
    ["OG0021_V3_Q03", "fixed_option_score", "score = opts[selected].score_value", "Initiating Event = Q03 score", "오답도 발단 장면과의 의미 거리로 부분 점수"],
    ["OG0021_V3_Q04", "weighted_word_position", "score = round(sum(weight[word] if word is in exact position) / sum(weights) * 100)", "Attempt = Q04 score", "원문 SC03_ST01_N 문장 그대로 사용"],
    ["OG0021_V3_Q05", "fixed_option_score", "score = opts[selected].score_value", "Reaction = Q05 score", "감정 라벨의 유사도에 따라 부분 점수"],
    ["OG0021_V3_Q06", "fixed_option_score", "score = opts[selected].score_value", "Internal Response = Q06 score", "보기형으로 내면 생각을 직접 선택"],
    ["OVERALL", "composite", "overall = average(Setting..Consequence)", "Parent report headline score", "Story Grammar 6축 평균으로 종합점수 산출"],
  ];
  ...
  V3_SG.forEach(axis => metricRows.push([axis.key, axis.label, axis.kor, "OG0021_V3_Q0" + (axis.q + 1), "radar_axis", "direct_question_score", "feedbackLine('" + axis.key + "', score)"]));
  metricRows.push(["overall", "Overall Reading Score", "종합 독해 점수", "Q01-Q06", "headline_score", "average(6 axes)", "overall band"]);
  ...
  XLSX.writeFile(wb, "OG0021_v3_DevSpec.xlsx");
};
