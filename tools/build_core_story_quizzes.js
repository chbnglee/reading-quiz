...
.progressive-steps{display:grid;grid-template-rows:repeat(3,70px);gap:12px;min-height:234px}
.progressive-step{display:grid;grid-template-columns:112px 1fr;gap:14px;align-items:stretch;opacity:1;visibility:visible;transform:translateY(0);transition:opacity .22s ease,transform .28s ease,visibility 0s linear 0s;overflow:hidden;border-top:1px solid #EDE9FE;padding-top:12px}
.progressive-step:first-child{border-top:0}
.progressive-step.is-hidden{opacity:0;visibility:hidden;transform:translateY(-10px);pointer-events:none;transition:opacity .18s ease,transform .28s ease,visibility 0s linear .28s}
